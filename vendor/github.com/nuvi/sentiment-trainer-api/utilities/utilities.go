...
import (
	...
	"github.com/nuvi/sentiment-trainer-api/postgres"
...
// Tokenize creates an array of words from a sentence
...
func tokenizeMulti(sentence string, size int, words []string) []string {
	var tokens []string
...
//CountWords counts individual and ngram of 2 words and returns a map
func CountWords(document string) (wordCount map[string]float64) {
	words := Tokenize(document)
	words = append(words, tokenizeMulti(document, 2, words)...)
...
//ParseJSON parses text into sentiment data
func ParseJSON(text []byte) (d postgres.SentimentData) {
